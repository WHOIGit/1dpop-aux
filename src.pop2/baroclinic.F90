...
!
! !REVISION HISTORY:
!  SVN:$Id: baroclinic.F90 82944 2017-01-20 22:55:30Z mlevy@ucar.edu $
...
!   use distribution, only:
   ...
   use grid, only: FCOR, DZU, HUR, KMU, KMT, sfc_layer_type, lpencil_model,  &
       sfc_layer_varthick, partial_bottom_cells, dz, DZT, CALCT, dzw, dzr,   &
       lsingle_col_force_f0d, lsingle_col_force_f1d, f0d_filename,           &
       f0d_fieldlistfile, f0d_fieldlistmodel, f1d_filename,                  &
       f1d_fieldlistfile, f1d_fieldlistmodel
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      ...
      tavg_FX1,          & ! U, V restoring diagnostics
      tavg_FX2,          & ! U, V restoring diagnostics
      tavg_FY1,          & ! U, V restoring diagnostics
      tavg_FY2,          & ! U, V restoring diagnostics
      tavg_UVEL3,        & ! U, V restoring diagnostics
      tavg_UVEL4,        & ! U, V restoring diagnostics
      tavg_UVEL5,        & ! U, V restoring diagnostics
      tavg_UVEL6,        & ! U, V restoring diagnostics
      tavg_VVEL3,        & ! U, V restoring diagnostics
      tavg_VVEL4,        & ! U, V restoring diagnostics
      tavg_VVEL5,        & ! U, V restoring diagnostics
      tavg_VVEL6           ! U, V restoring diagnostics
...
!-----------------------------------------------------------------------
   ...
   type(strdata_input_type) :: &
      ...
      F0d_inputlist       ! pop stream datatype for 1d vertical forcing data
   ...
   integer (int_kind) :: &
      ...
      F0d_QICE_TEMP_ind, &! index into the stream av for QICE_TEMP
      F0d_QICE_SALT_ind   ! index into the stream av for QICE_SALT
...
!  U, V restoring diagnotics

   call define_tavg_field(tavg_FX1,'FX1',3,                            &
                          long_name='Source term in grid-x direction', &
                          units='centimeter/sec^2', grid_loc='3221',   &
                          coordinates='ULONG ULAT z_t time')

   call define_tavg_field(tavg_FX2,'FX2',3,                            &
                          long_name='Source term in grid-x direction', &
                          units='centimeter/sec^2', grid_loc='3221',   &
                          coordinates='ULONG ULAT z_t time')

   call define_tavg_field(tavg_FY1,'FY1',3,                            &
                          long_name='Source term in grid-y direction', &
                          units='centimeter/sec^2', grid_loc='3221',   &
                          coordinates='ULONG ULAT z_t time')

   call define_tavg_field(tavg_FY2,'FY2',3,                            &
                          long_name='Source term in grid-y direction', &
                          units='centimeter/sec^2', grid_loc='3221',   &
                          coordinates='ULONG ULAT z_t time')

   call define_tavg_field(tavg_UVEL3,'UVEL3',3,                        &
                          long_name='Velocity in grid-x direction',    &
                          units='centimeter/s', grid_loc='3221',       &
                          coordinates='ULONG ULAT z_t time')

   call define_tavg_field(tavg_UVEL4,'UVEL4',3,                        &
                          long_name='Velocity in grid-x direction',    &
                          units='centimeter/s', grid_loc='3221',       &
                          coordinates='ULONG ULAT z_t time')

   call define_tavg_field(tavg_UVEL5,'UVEL5',3,                        &
                          long_name='Velocity in grid-x direction',    &
                          units='centimeter/s', grid_loc='3221',       &
                          coordinates='ULONG ULAT z_t time')

   call define_tavg_field(tavg_UVEL6,'UVEL6',3,                        &
                          long_name='Velocity in grid-x direction',    &
                          units='centimeter/s', grid_loc='3221',       &
                          coordinates='ULONG ULAT z_t time')

   call define_tavg_field(tavg_VVEL3,'VVEL3',3,                        &
                          long_name='Velocity in grid-y direction',    &
                          units='centimeter/s', grid_loc='3221',       &
                          coordinates='ULONG ULAT z_t time')

   call define_tavg_field(tavg_VVEL4,'VVEL4',3,                        &
                          long_name='Velocity in grid-y direction',    &
                          units='centimeter/s', grid_loc='3221',       &
                          coordinates='ULONG ULAT z_t time')

   call define_tavg_field(tavg_VVEL5,'VVEL5',3,                        &
                          long_name='Velocity in grid-y direction',    &
                          units='centimeter/s', grid_loc='3221',       &
                          coordinates='ULONG ULAT z_t time')

   call define_tavg_field(tavg_VVEL6,'VVEL6',3,                        &
                          long_name='Velocity in grid-y direction',    &
                          units='centimeter/s', grid_loc='3221',       &
                          coordinates='ULONG ULAT z_t time')
...
!-----------------------------------------------------------------------
   ...
   if (lpencil_model) then
      ...
      call POP_strdata_init(F0d_inputlist)
      call POP_strdata_init(F1d_inputlist)

      F0d_inputlist%field_name = 'F0d data'
      F0d_inputlist%short_name = 'F0d'
      F0d_inputlist%year_first = 1
      F0d_inputlist%year_last  = 1
      F0d_inputlist%year_align = 1
      F0d_inputlist%file_name  = trim(f0d_filename)
      F0d_inputlist%field_list_file  = trim(f0d_fieldlistfile)
      F0d_inputlist%field_list_model = trim(f0d_fieldlistmodel)
      F0d_inputlist%dtlimit    = 1.e30
      ...
      F0d_QICE_TEMP_ind = shr_string_listGetIndexF(F0d_inputlist%field_list_model,'QICE_TEMP')
      F0d_QICE_SALT_ind = shr_string_listGetIndexF(F0d_inputlist%field_list_model,'QICE_SALT')

      lsingle_col_force_f0d = .false.
      if (F0d_QICE_TEMP_ind > 0 .or. F0d_QICE_SALT_ind > 0) lsingle_col_force_f0d = .true.
      lsingle_col_force_f1d = .false.
      if (F1d_GT_ind > 0 .or. F1d_GS_ind > 0 .or. F1d_GU_ind > 0 .or. F1d_GV_ind > 0) lsingle_col_force_f1d=.true.

      call get_timer(F1d_shr_strdata_advance_timer, &
                     'F1D_SHR_STRDATA_ADVANCE',1, distrb_clinic%nprocs)
      if (my_task == master_task) then
         write(stdout,blank_fmt)
         write(stdout,'(a,l4)') ' 1d f0d forcing data shr_stream: ',lsingle_col_force_f0d
         write(stdout,'(2a)')   '   f0d_filename        = ',trim(F0d_inputlist%file_name)
         write(stdout,'(2a)')   '   f0d_file_list_file  = ',trim(F0d_inputlist%field_list_file)
         write(stdout,'(2a)')   '   f0d_file_list_model = ',trim(F0d_inputlist%field_list_model)
         write(stdout,'(a,i6)') '   QICE_TEMP index = ',F0d_QICE_TEMP_ind
         write(stdout,'(a,i6)') '   QICE_SALT index = ',F0d_QICE_SALT_ind
...
!-----------------------------------------------------------------------
   ...
   if (lsingle_col_force_f0d .or. lsingle_col_force_f1d) then
      if (first_call_strdata_create) then
         if (lsingle_col_force_f0d) &
            call POP_strdata_create(F0d_inputlist,depthflag=.false., &
            XvarName='TLONG',YvarName='TLAT',TvarName='time')
      ...
      if (lsingle_col_force_f0d) then
         F0d_inputlist%date = iyear*10000 + imonth*100 + iday
         F0d_inputlist%time = isecond + 60 * (iminute + 60 * ihour)
         call POP_strdata_advance(F0d_inputlist)
         ...
         do iblock = 1, nblocks_clinic
            ...
            do i=this_block%ib,this_block%ie
               n = n + 1
               if (F0d_QICE_TEMP_ind > 0) &
                  FQICE_TEMP(i,j,iblock) = F0d_inputlist%sdat%avs(1)%rAttr(F0d_QICE_TEMP_ind,n)
               if (F0d_QICE_SALT_ind > 0) &
                  FQICE_SALT(i,j,iblock) = F0d_inputlist%sdat%avs(1)%rAttr(F0d_QICE_SALT_ind,n)
!               write(stdout,*) 'FQT = ',i,j,iblock,FQICE_TEMP(i,j,iblock)
!               write(stdout,*) 'FQS = ',i,j,iblock,FQICE_SALT(i,j,iblock)
            enddo
            enddo
         enddo
         enddo
...
! BT 17 Apr 17 uncmommented write statements
!               write(stdout,*) 'FGT = ',i,j,k,iblock,FGT(i,j,k,iblock)
!               write(stdout,*) 'FGS = ',i,j,k,iblock,FGS(i,j,k,iblock)
!               write(stdout,*) 'FGU = ',i,j,k,iblock,FGU(i,j,k,iblock)
!               write(stdout,*) 'FGV = ',i,j,k,iblock,FGV(i,j,k,iblock)
            enddo
            enddo
...
! BT 17 Apr 17
!            write(101,*) 'FGT = ',iblock,i,j,k,FGT(3:nx_block-2, 3:ny_block-2,k,iblock)
!            write(102,*) 'FGS = ',iblock,i,j,k,FGS(3:nx_block-2, 3:ny_block-2,k,iblock)
!            write(103,*) 'FGU = ',iblock,i,j,k,FGU(3:nx_block-2, 3:ny_block-2,k,iblock)
!            write(104,*) 'FGV = ',iblock,i,j,k,FGV(3:nx_block-2, 3:ny_block-2,k,iblock)
...
!-----------------------------------------------------------------------
         ...
         call accumulate_tavg_field(UVEL(:,:,k,newtime,iblock),tavg_UVEL3,iblock,k)
         call accumulate_tavg_field(VVEL(:,:,k,newtime,iblock),tavg_VVEL3,iblock,k)
...
!-----------------------------------------------------------------------
      ...
      ! U, V restoring diagnostics
      do k = 1,km
         call accumulate_tavg_field(UVEL(:,:,k,newtime,iblock),tavg_UVEL4,iblock,k)
         call accumulate_tavg_field(VVEL(:,:,k,newtime,iblock),tavg_VVEL4,iblock,k)
      enddo
...
!-----------------------------------------------------------------------
      ...
      ! U, V restoring diagnostics
      do k = 1,km
         call accumulate_tavg_field(UVEL(:,:,k,newtime,iblock),tavg_UVEL5,iblock,k)
         call accumulate_tavg_field(VVEL(:,:,k,newtime,iblock),tavg_VVEL5,iblock,k)
      enddo
...
!-----------------------------------------------------------------------
      ...
      if (mix_pass /= 1) then
         call impvmixt_tavg(TRACER(:,:,:,:,newtime,iblock),                   &
                            STF(:,:,:,iblock), iblock)
         call impvmixu_tavg(UVEL(:,:,:,newtime,iblock),                       &
                            VVEL(:,:,:,newtime,iblock),                       &
                            SMF(:,:,:,iblock), iblock)
...
! BT 22 Mar 17     call accumulate_tavg_field(WORK, tavg_TEND_TRACER(n), iblock, k) ; including this caused the build to fail.  need more code to get it to work
               if (n.eq.1) then
                 call accumulate_tavg_field(WORK, tavg_id('GT'), iblock, k)
               end if
               if (n.eq.2) then
                 call accumulate_tavg_field(WORK, tavg_id('GS'), iblock, k)
               end if
            end do
        end do
      endif
...
!-----------------------------------------------------------------------
   ...
   ! U, V restoring diagnostics
   call accumulate_tavg_field(FX,tavg_FX1,bid,k)
   call accumulate_tavg_field(FY,tavg_FY1,bid,k)
...
!-----------------------------------------------------------------------
   ...
   FY = FY + WORKY
   ! U, V restoring diagnostics
   call accumulate_tavg_field(WORKX,tavg_FX2,bid,k)
   call accumulate_tavg_field(WORKY,tavg_FY2,bid,k)

  ! add internal forcing terms
   if (lsingle_col_force_f1d) then
...
!  add restoring terms - Ivan Lima - old location
!   call set_uv_interior(k,this_block,WORKX,WORKY)
!   FX = FX + WORKX
!   FY = FY + WORKY
...
!-----------------------------------------------------------------------
   ...
   ! add in internal forcing terms
   if (lsingle_col_force_f0d .and. k == 1) then
      if (F0d_QICE_TEMP_ind > 0) WORKN(:,:,1) = WORKN(:,:,1) + FQICE_TEMP(:,:,bid)
      if (F0d_QICE_SALT_ind > 0) WORKN(:,:,2) = WORKN(:,:,2) + FQICE_SALT(:,:,bid)
   endif

   if (lsingle_col_force_f1d) then
      if (F1d_GT_ind > 0) then
         FGT(:,:,k,bid) = merge(FGT(:,:,k,bid), c0, FGT(:,:,k,bid) <= 1.e30)  ! BT 23 Feb 17 don't need anymore -doing this when terms are read in?
         where (k <= KMT(:,:,bid))  WORKN(:,:,1) = WORKN(:,:,1) + FGT(:,:,k,bid)
      endif

      if (F1d_GS_ind > 0) then
         FGS(:,:,k,bid) = merge(FGS(:,:,k,bid), c0, FGS(:,:,k,bid) <= 1.e30)  ! BT 23 Feb 17 don't need anymore -doing this when terms are read in?
