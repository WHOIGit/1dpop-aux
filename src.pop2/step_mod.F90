...
!
! !REVISION HISTORY:
!  SVN:$Id: step_mod.F90 81500 2016-11-04 15:40:40Z tomas $
!
! !USES:
...
!-----------------------------------------------------------------------
   ...
   real (POP_r8), dimension(nx_block,ny_block) :: &
      FU, FV,             &! k-level of Coriolis terms
      TEND_U, TEND_V       ! k-level of velocity tendencies
   ...
   real (POP_r8) :: tmp_convert1, & ! dummy variables to convert from
                    tmp_convert2    ! (1) W/m^2 -> deg C/s and
                                    ! (2) W/m^2 -> g/kg/s
!-----------------------------------------------------------------------
!
!  start step timer
...
!-----------------------------------------------------------------------
   ...
   tmp_convert1 = 1e3_POP_r8/(rho_sw*cp_sw*dz(1))
   tmp_convert2 = 1e3_POP_r8*0.0307_POP_r8/(latent_heat_fusion*rho_fw*dz(1))
...
!-----------------------------------------------------------------------
   ...
   !$OMP PARALLEL DO PRIVATE(iblock,TEND_U,TEND_V,FU,FV)
   do iblock = 1,nblocks_clinic
   ! Output Coriolis and time tendencies before shuffling time indices
   ! (And before time averaging)
   ! dX/dt = (X(new) - X(old))/(2*dt)
      do k=1,km
! BT 14 June 17 Mike alerted me that I shouldn't have the p5
!        TEND_U = p5*(UVEL(:,:,k,newtime,iblock) - UVEL(:,:,k,oldtime,iblock))/c2dtu
!        TEND_V = p5*(VVEL(:,:,k,newtime,iblock) - VVEL(:,:,k,oldtime,iblock))/c2dtu

        TEND_U = (UVEL(:,:,k,newtime,iblock) - UVEL(:,:,k,oldtime,iblock))/c2dtu
        TEND_V = (VVEL(:,:,k,newtime,iblock) - VVEL(:,:,k,oldtime,iblock))/c2dtu



        FU     = p33*FCOR(:,:,iblock)*sum(UVEL(:,:,k,:,iblock), dim=3)
        FV     = p33*FCOR(:,:,iblock)*sum(VVEL(:,:,k,:,iblock), dim=3)
        call accumulate_tavg_field(TEND_U, tavg_id('GU'), iblock, k)
        call accumulate_tavg_field(   -FV, tavg_id('GU'), iblock, k)
        call accumulate_tavg_field(TEND_V, tavg_id('GV'), iblock, k)
        call accumulate_tavg_field(    FU, tavg_id('GV'), iblock, k)
      end do ! k loop
   end do ! block loop
   !$OMP END PARALLEL DO
...
!-----------------------------------------------------------------------
   ...
   if ( liceform .and. check_time_flag(ice_cpl_flag) ) then
     ...
     do iblock = 1,nblocks_clinic
        ...
        call accumulate_tavg_field(QFLUX(:,:,iblock)*tmp_convert1,       &
                                   tavg_id('QICE_TEMP'),iblock,1,        &
                                   const=tlast_ice)
        call accumulate_tavg_field(QFLUX(:,:,iblock)*tmp_convert2,       &
                                   tavg_id('QICE_SALT'),iblock,1,        &
                                   const=tlast_ice)
     end do ! block loop
     !$OMP END PARALLEL DO
!-----------------------------------------------------------------------
