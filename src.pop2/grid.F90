...
!
! !REVISION HISTORY:
!  SVN:$Id: grid.F90 82944 2017-01-20 22:55:30Z mlevy@ucar.edu $
...
! !PUBLIC DATA MEMBERS:
   ...
   real (POP_r8), public :: &
      ...
      Coriolis_min           ! smallest value for Coriolis (if lmin_Coriolis=.true.)
   ...
   logical (POP_logical), public ::    &
      ...
      lpencil_model,           &! flag to run POP in 1D mode (experimental)
      lsingle_col,             &! flag to specify nx_global = ny_global = 1
      lsingle_col_force_f0d,   &! flag to specify single column f0d external forcing is turned on
      lsingle_col_force_f1d,   &! flag to specify single column f1d external forcing is turned on
      lmin_Coriolis             ! flag to specify minimum for Coriolis

   character(char_len),   public ::     &
      f0d_filename,            &! 1D mode input file for surface fields
      f0d_fieldlistfile,       &! 1D mode file field list for surface fields
      f0d_fieldlistmodel,      &! 1D mode model field list for surface fields
...
!-----------------------------------------------------------------------
   ...
   namelist /pencil_model_nml/f0d_filename, f0d_fieldlistfile, f0d_fieldlistmodel, &
                              f1d_filename, f1d_fieldlistfile, f1d_fieldlistmodel, &
                              lmin_Coriolis, Coriolis_min
...
!-----------------------------------------------------------------------
   ...
   kmt_kmin             = 3
   lpencil_model        = .false.
   lsingle_col          = (nx_global.eq.1).and.(ny_global.eq.1)
   lsingle_col_force_f0d= lsingle_col
   lsingle_col_force_f1d= lsingle_col
   f0d_filename         = 'unknown_f0d_filename'
   f0d_fieldlistfile    = ''
   f0d_fieldlistmodel   = ''
   ...
   if (my_task == master_task) then
      ...
      do while (nml_error > 0)
         read(nml_in, nml=pencil_model_nml,iostat=nml_error)
      end do
   ...
   if (my_task == master_task) then
      ...
      write(stdout, pencil_model_nml)
      if (lpencil_model) &
        write(stdout,*) 'NOTE: running in pencil mode!'
        write(stdout,*) '  f0d_filename         =',trim(f0d_filename)
        write(stdout,*) '  f0d_fieldlistfile    =',trim(f0d_fieldlistfile)
        write(stdout,*) '  f0d_fieldlistmodel   =',trim(f0d_fieldlistmodel)
        write(stdout,*) '  f1d_filename         =',trim(f1d_filename)
        write(stdout,*) '  f1d_fieldlistfile    =',trim(f1d_fieldlistfile)
        write(stdout,*) '  f1d_fieldlistmodel   =',trim(f1d_fieldlistmodel)
      if (lmin_Coriolis) &
        write(stdout,*) 'NOTE: abs(Coriolis) must be no smaller than', Coriolis_min
   endif
   ...
   call broadcast_scalar(lpencil_model,        master_task)
   call broadcast_scalar(f0d_filename,         master_task)
   call broadcast_scalar(f0d_fieldlistfile,    master_task)
   call broadcast_scalar(f0d_fieldlistmodel,   master_task)
