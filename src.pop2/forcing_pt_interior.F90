...
! Time-stamp: <2020-06-01 09:44:36 ivan>
...
!
! !REVISION HISTORY:
!  SVN:$Id: forcing_pt_interior.F90 72615 2015-08-26 23:32:40Z mlevy@ucar.edu $
!
! !USES
   ...
   use exit_mod
   use tavg
   use forcing_sst, only : sst_alpha
...
!-----------------------------------------------------------------------
   ...
   integer (int_kind) :: &
      tavg_PT_INTERIOR,  &! tavg_id for surface heat flux
      tavg_PT_INTERIOR_2  ! tavg_id for surface heat flux
...
!-----------------------------------------------------------------------
   ...
   case ('monthly-equal','monthly-calendar')
      ...
      ! write(stdout,'(a28,12(f12.6,1X))') 'IVAN pt_interior_data_time ', pt_interior_data_time
      ! write(stdout,'(a30,a20)') 'IVAN pt_interior_interp_type ', pt_interior_interp_type
      ! write(stdout,*) 'IVAN pt_interior_data_time_min_loc', pt_interior_data_time_min_loc
      ! write(stdout,'(a30,a20)') 'IVAN pt_interior_interp_freq ', pt_interior_interp_freq
      ! write(stdout,*) 'IVAN pt_interior_interp_inc ', pt_interior_interp_inc
      ! write(stdout,*) 'IVAN pt_interior_interp_next ', pt_interior_interp_next
      ! write(stdout,*) 'IVAN pt_interior_interp_last ', pt_interior_interp_last
      ! write(stdout,*) 'IVAN pt_nsteps_run ', nsteps_run
      ! write(stdout,*) 'IVAN -------------------------------------------------------------------'
...
!      write(stdout,*) 'IVAN', pt_interior_surface_restore, pt_interior_data_type
...
!     check interpolated reference fields
!      call accumulate_tavg_field(PT_INTERIOR_DATA(:,:,k,bid,now), tavg_PT_INTERIOR, bid, k)
